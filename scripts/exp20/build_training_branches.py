"""Freeze four reference-only branch landmarks per EXP8 training demo."""
...
def main():
    ...
    artifacts=out/"artifacts";artifacts.mkdir(parents=True);reference=ROOT/args.reference_run;records=json.loads((reference/"artifacts/reference_snapshots_manifest.json").read_text())["episodes"];schema=load_schema(ROOT/"experiments/exp7_contact_mode_conditioned/manifests/contact_mode_schema.json");branches=[]
    ...
    dump(artifacts/"branch_manifest.json",branches);metrics={"status":"completed","branch_count":len(branches),"demo_count":len(records),"branches_per_demo":4,"selection":"reference-only EXP11 landmarks","frozen_before_candidate_outcomes":True};dump(out/"metrics.json",metrics);print(json.dumps(metrics,indent=2));return 0
