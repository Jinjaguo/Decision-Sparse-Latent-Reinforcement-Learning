...
def main()->int:
    ...
    for name in args.source_runs:
        source=args.run_root/name; metrics_path=source/"metrics.json"; top_i=source/"artifacts/interventions.parquet"; top_e=source/"artifacts/per_step_effects.parquet"
        if metrics_path.exists() and json.loads(metrics_path.read_text()).get("gate",{}).get("passed") and top_i.exists():
            candidates=[(top_i,top_e)]; sources[name]=sha(metrics_path)
        else:
            candidates=[(path,path.with_name("per_step_effects.parquet")) for path in sorted((source/"artifacts/shards").glob("*/*/interventions.parquet"))]; sources[name]="recovered immutable per-demonstration shards after explicit process repartition"
        if not candidates: raise RuntimeError(f"no complete immutable shards in {name}")
        for ip,ep in candidates:
            if not ep.exists(): raise RuntimeError(f"missing paired per-step artifact for {ip}")
            ips.append(ip); eps.append(ep)
    ...
    criteria={"exact_17280":len(rows)==17280,"unique_rows":len(actual)==len(rows),"exact_frozen_coverage":actual==expected,"all_sources_nonempty":bool(ips),"no_failures":not failures,"geometry_fields":{"zero_contact_mode_json","perturbed_contact_mode_json","zero_signed_gap_m","perturbed_signed_gap_m"}.issubset(schema)}
    hashes={name:sha(art/name) for name in ("zero_controls.parquet","zero_reference_steps.parquet","interventions.parquet","per_step_effects.parquet")}; write_json(art/"raw_data_lock_manifest.json",{"locked_before_analysis":True,"source_metrics_sha256":sources,"sha256":hashes}); write_json(art/"failure_examples.json",failures[:100]); metrics={"run_id":args.run_id,"status":"completed","gate":{"passed":all(criteria.values()),"criteria":criteria},"intervention_count":len(rows),"per_step_effect_count":pq.read_metadata(art/"per_step_effects.parquet").num_rows,"success_flip_count":sum(x["success_flip"] for x in rows),"maximum_non_arm_linf":max(x["non_arm_max_linf"] for x in rows),"maximum_q_injection_error":max(x["q_injection_max_abs_error"] for x in rows),"raw_hashes":hashes}; write_run_record(run,config={"stage":"EXP7 raw merge and lock","sources":args.source_runs},command=shlex.join([sys.executable,*sys.argv]),environment={"python":sys.version,"pyarrow":pa.__version__},git_state={"project":git_record(ROOT)},metrics=metrics,stdout=json.dumps(metrics),stderr=""); print(json.dumps(metrics,indent=2)); return 0 if metrics["gate"]["passed"] else 2
