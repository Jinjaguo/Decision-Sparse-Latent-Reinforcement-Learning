...
def main() -> int:
    ...
    try:
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
                    ...
                    if args.mode in ("full", "dry"):
                        ...
                        for direction in rows:
                            for sign in config["signs"]:
                                radius = float(direction.get("radius_fraction", config.get("joint_range_fraction", 0.005))); radius_label = str(direction.get("radius_label", f"r{radius:g}")); signed = int(sign) * np.asarray(direction["unsigned_delta_q"], dtype=np.float64); intervention_id = f"{trajectory['task']}|{trajectory['episode']}|{branch_time}|{radius_label}|d{direction['direction_index']}|s{int(sign):+d}"; ibase = {**base, "intervention_id": intervention_id, "direction_index": int(direction["direction_index"]), "direction_role": direction["direction_role"], "execution_position": int(direction["execution_position"]), "radius_fraction": radius, "radius_label": radius_label, "sign": int(sign)}
            ...
            expected_interventions = 2 * sum(len(rows) for rows in expected_direction_rows.values())
            if args.mode == "zero":
                ...
                for path in intervention_shards:
                    for x in pq.read_table(path).to_pylist(): key = (x["task"], x["episode"], x["branch_time"]); keys.add(key); coverage.setdefault(key, set()).add((x.get("radius_label", "legacy"), x["direction_index"], x["sign"]))
                expected_coverage = {key: {(str(row.get("radius_label", "legacy")), int(row["direction_index"]), int(sign)) for row in rows for sign in config["signs"]} for key, rows in expected_direction_rows.items()}
