...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--zero-repeats", type=int, default=2)
    parser.add_argument("--intervention-repeats", type=int, default=1)
...
def main() -> int:
    ...
    if args.intervention_repeats < 1: raise ValueError("intervention-repeats must be positive")
    run_config = {"run_id": args.run_id, "stage": f"{experiment_name}_{args.mode}", "mode": args.mode, "reference_run": reference.name, "zero_run": None if args.zero_run is None else args.zero_run.resolve().name, "manifest_dir": str(manifest_dir), "trajectory_start": args.trajectory_start, "max_trajectories": args.max_trajectories, "max_branches": args.max_branches, "max_directions": args.max_directions, "zero_repeats": args.zero_repeats, "intervention_repeats": args.intervention_repeats, "fixed_config": config, "sharding": "one immutable task/demo shard written before merge"}
        ...
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            ...
            for trajectory in trajectories:
                ...
                for branch in branches:
                    base = exp3.identity(trajectory, branch); base["physical_progress_raw"] = branch.get("physical_progress_raw", branch.get("normalized_time", 0.0)); base["physical_progress_clipped"] = branch.get("physical_progress_clipped", branch.get("normalized_time", 0.0)); base["state_prototype_id"] = branch.get("state_prototype_id"); base["crossfit_fold"] = branch.get("crossfit_fold")
                    for key in ("reference_contact_state", "reference_gripper_state", "reference_predicate_state", "reference_stratum", "selection_distance", "deterministic_replacement_reason"):
                        base[key] = branch.get(key)
                    branch_time = base["branch_time"]; controller = ref_dir / f"controller_{branch_time:04d}.npz"
                    if args.mode in ("zero", "dry"):
                        twins = []
                        for _ in range(args.zero_repeats):
                            exp3.restore_d(env, integrations[branch_time], controller); twins.append(exp3.rollout(env, actions, branch_time, body_ids))
                        twin_a = twins[0]; differences = [exp3.compare_zero(a, b) for twin in twins[1:] for a, b in zip(twin_a, twin)]
                        ...
                        zsum.append({**base, "zero_continuation_ids": [f"{trajectory['task']}|{trajectory['episode']}|{branch_time}|Z{i}" for i in range(args.zero_repeats)], "zero_repeat_count": args.zero_repeats, "maximum_integration_l2": max(x["integration_l2"] for x in differences), "median_integration_l2": float(np.median([x["integration_l2"] for x in differences])), "p95_integration_l2": float(np.percentile([x["integration_l2"] for x in differences], 95)), "maximum_arm_q_l2": max(x["arm_q_l2"] for x in differences), "maximum_arm_qvel_l2": max(x["arm_qvel_l2"] for x in differences), "maximum_eef_position_l2": max(x["eef_position_l2"] for x in differences), "maximum_eef_orientation_geodesic": max(x["eef_orientation_geodesic"] for x in differences), "maximum_task_object_position_l2": max(x["task_object_position_l2"] for x in differences), "maximum_task_object_orientation_geodesic_mean": max(x["task_object_orientation_geodesic_mean"] for x in differences), "maximum_contact_pair_symmetric_difference_count": max(x["contact_pair_symmetric_difference_count"] for x in differences), "maximum_raw_contact_count_difference": max(x["raw_contact_count_difference"] for x in differences), "any_task_predicate_divergence": any(x["task_predicate_divergence"] for x in differences), "terminal_object_pose_l2": terminal_object, "terminal_success_a": twin_a[-1]["predicate"], "terminal_success_b": twins[-1][-1]["predicate"], "all_states_finite": all(np.all(np.isfinite(x["integration"])) for twin in twins for x in twin)})
                    ...
                    if args.mode in ("full", "dry"):
                        ...
                        for direction in rows:
                            for sign in config["signs"]:
                                for repeat_index in range(args.intervention_repeats):
                                    radius = float(direction.get("radius_fraction", config.get("joint_range_fraction", 0.005))); radius_label = str(direction.get("radius_label", f"r{radius:g}")); signed = int(sign) * np.asarray(direction["unsigned_delta_q"], dtype=np.float64); intervention_id = f"{trajectory['task']}|{trajectory['episode']}|{branch_time}|{radius_label}|d{direction['direction_index']}|s{int(sign):+d}|r{repeat_index}"; ibase = {**base, "intervention_id": intervention_id, "repeat_index": repeat_index, "direction_index": int(direction["direction_index"]), "direction_role": direction["direction_role"], "execution_position": int(direction["execution_position"]), "radius_fraction": radius, "radius_label": radius_label, "sign": int(sign)}
                                    exp3.restore_d(env, integrations[branch_time], controller); before = mujoco_snapshot.capture_atomic_components(env.sim); q_before = np.asarray(env.sim.data.qpos[indexes]).copy(); q_after = apply_arm_q(env.sim.data, indexes, signed, lower, upper); after = mujoco_snapshot.capture_atomic_components(env.sim); preservation = non_arm_integration_linf(before, after, indexes); injection_error = float(np.max(np.abs((q_after - q_before) - signed))); perturb = exp3.rollout(env, actions, branch_time, body_ids); effects = []
                                    for offset, observation in enumerate(perturb):
                                        zero = zero_lookup[(trajectory["task"], trajectory["episode"], branch_time, offset)] if args.mode == "full" else twin_a[offset]; row = exp3.effect_row(ibase, offset, zero, observation, normalization); row["zero_contact_pairs_json"] = json.dumps(zero["contacts"], sort_keys=True); row["perturbed_contact_pairs_json"] = json.dumps(observation["contacts"], sort_keys=True); vector = signed_output_vector(zero, observation, normalization, trajectory["task"], joint_ranges); row["signed_normalized_physical_output_vector"] = vector.tolist(); esteps.append(row); effects.append(row)
            ...
            expected_direction_rows = {(t["task"], t["episode"], int(b["action_index"])): direction_lookup[(t["task"], t["episode"], int(b["action_index"]))][:args.max_directions] for t in trajectories for b in t["branches"][:args.max_branches]}
            expected_interventions = len(config["signs"]) * args.intervention_repeats * sum(len(rows) for rows in expected_direction_rows.values())
            if args.mode == "zero":
                ...
                criteria = {"all_expected_branches": zero_branch_count == expected_branches, "required_zero_continuations_each": all(len(x["zero_continuation_ids"]) == args.zero_repeats for path in zero_shards for x in pq.read_table(path).to_pylist()), "median_gate": stats["median"] <= config["zero_median_max"], "p95_gate": stats["p95"] <= config["zero_p95_max"], "maximum_gate": stats["maximum"] <= config["zero_maximum_max"], "terminal_object_pose_p95_gate": stats["terminal_object_pose_p95"] <= config["zero_terminal_object_pose_p95_max"], "expected_exact_zero": stats["maximum"] == 0.0, "no_failures": not all_failures}
            elif args.mode == "full":
                keys = set(); coverage = {}
                for path in intervention_shards:
                    for x in pq.read_table(path).to_pylist(): key = (x["task"], x["episode"], x["branch_time"]); keys.add(key); coverage.setdefault(key, set()).add((x.get("radius_label", "legacy"), x["direction_index"], x["sign"], x.get("repeat_index", 0)))
                expected_coverage = {key: {(str(row.get("radius_label", "legacy")), int(row["direction_index"]), int(sign), repeat_index) for row in rows for sign in config["signs"] for repeat_index in range(args.intervention_repeats)} for key, rows in expected_direction_rows.items()}
                criteria = {"all_expected_interventions": intervention_count == expected_interventions, "all_expected_branches": len(keys) == expected_branches, "all_directions_radii_two_signs_each": all(coverage.get(key, set()) == values for key, values in expected_coverage.items()), "only_arm_q_changed": global_nonarm <= config["non_arm_integration_linf_max"], "joint_limits_valid": not all_failures, "all_finite": not all_failures, "no_branch_removed": len(keys) == expected_branches}
                stats = {}
            else:
