"""Evaluate the frozen EXP24 task-modular coordinator."""
...
PRIMARY="T0_task_modular_primary"
...
def main():
    ...
    artifacts=out/"artifacts";artifacts.mkdir(parents=True);rows=pq.read_table(ROOT/args.candidate_run/"artifacts/candidate_summaries.parquet").to_pylist();routes=sorted(set(x["route"] for x in rows));branches=sorted(set(x["branch_id"] for x in rows));lookup={(x["branch_id"],x["route"]):x for x in rows};selected=[];defaults=[];oracles=[]
    for bid in branches:
