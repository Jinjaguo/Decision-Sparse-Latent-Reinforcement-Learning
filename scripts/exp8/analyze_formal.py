...
def main() -> int:
    ...
    risk = risk_base.merge(branch_features, on=["task", "episode", "branch_time"]).merge(action_features, on=["task", "episode", "branch_time", "radius_fraction", "direction_index", "direction_role"]).merge(folds, on=["task", "episode"])
    risk["risk_features"] = risk.apply(lambda row: np.concatenate((float_array(row.baseline_b_features), float_array(row.action_projection_features), [row.radius_fraction, row.sign, row.direction_index / 7, float(row.direction_role == "heldout_random")])), axis=1)
