...
def main() -> int:
    ...
    h1 = bool(demo.improvement.mean() >= 0.15 and h1_ci[0] > 0 and all(value > 0 for value in per_task.values()) and sum(value[0] > 0 for value in per_task_ci.values()) >= 2)
    ...
    held_frame = pd.DataFrame(held_rows); demo_held = held_frame.groupby(["task", "episode"]).apply(lambda group: pd.Series({"rho": float(spearmanr(group.predicted_norm, group.actual_norm).statistic) if len(group) > 1 else 0.0, "median_error": float(group.vector_relative_error.median())})).reset_index(); tails = upper_tail(held_frame.vector_relative_error)
    h2 = bool(demo_held.rho.median() >= 0.70 and demo_held.median_error.median() <= 0.35 and tails["p90"] <= 0.60)
    ...
    for name, removed in remove_map.items():
        ...
        ablations.append({"ablation": name, "mean_top1": float(result.primary_top1.mean()), "mean_improvement_over_baseline_b": float(result.improvement_over_baseline_b.mean()), "delta_from_full": float(result.primary_top1.mean() - cross.primary_top1.mean())})
    ablations.extend([{"ablation": "exact_mode_margin_only", "mean_top1": float(cross.mode_margin_top1.mean()), "mean_improvement_over_baseline_b": float((cross.mode_margin_top1 - cross.baseline_b_top1).mean()), "delta_from_full": float(cross.mode_margin_top1.mean() - cross.primary_top1.mean())}, {"ablation": "EXP5_physical_state", "mean_top1": float(cross.baseline_b_top1.mean()), "mean_improvement_over_baseline_b": 0.0, "delta_from_full": float(cross.baseline_b_top1.mean() - cross.primary_top1.mean())}, {"ablation": "gap_normal_velocity_only", "mean_top1": float(cross.baseline_b_top1.mean()), "mean_improvement_over_baseline_b": 0.0, "delta_from_full": float(cross.baseline_b_top1.mean() - cross.primary_top1.mean())}, {"ablation": "exact_geom_id_vs_physical_group", "mean_top1": float(cross.mode_margin_top1.mean()), "mean_improvement_over_baseline_b": float((cross.mode_margin_top1 - cross.baseline_b_top1).mean()), "delta_from_full": float(cross.mode_margin_top1.mean() - cross.primary_top1.mean())}, {"ablation": "contact_count_diagnostic", "mean_top1": None, "mean_improvement_over_baseline_b": None, "delta_from_full": None}])
    h3_q = h3_p
    ...
    for horizon in ("1", "3", "5", "remaining"):
        ...
        conditional_frame = intent.merge(horizon_data[["task", "episode", "branch_time", "all_basis_both_signs_preserved"]], on=["task", "episode", "branch_time"])
        preservation = float(conditional_frame.all_basis_both_signs_preserved.mean()); conditional_similarity = float(conditional_frame.loc[conditional_frame.all_basis_both_signs_preserved, "primary_top1"].mean()) if conditional_frame.all_basis_both_signs_preserved.any() else 0.0
