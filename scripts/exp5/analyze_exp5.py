...
def main()->int:
 ...
 desc=pq.read_table(a.development_run.resolve()/'artifacts/development_state_descriptor.parquet').to_pylist(); dl={(x['task'],x['episode'],x['action_index']):np.asarray(x['descriptor']) for x in desc if x['cohort']=='confirmatory'}; schema=json.loads((a.manifest_dir/'state_descriptor_schema.json').read_text()); scaling=json.loads((a.manifest_dir/'state_descriptor_scaling.json').read_text())['tasks']; prot=json.loads((a.manifest_dir/'prototype_manifest.json').read_text())['records']; prec={(x['task'],x['fold']):np.asarray(x['precision']) for x in prot}; branch=json.loads((a.manifest_dir/'branch_manifest.json').read_text())['trajectories']; main=sdf[sdf.radius_label=='main']; mainop=odf[odf.radius_label=='main']; matches=[]
 for task in TASKS:
  ...
  for ai,ta in enumerate(demos):
   for tb in demos[ai+1:]:
    ba=sorted(ta['branches'],key=lambda x:x['action_index']); bb=sorted(tb['branches'],key=lambda x:x['action_index']); xa=np.asarray([dl[(task,ta['episode'],x['action_index'])] for x in ba]); xb=np.asarray([dl[(task,tb['episode'],x['action_index'])] for x in bb]); ss=scaling[task]; center=np.asarray(ss['center']); scale=np.asarray(ss['scale']); weight=np.asarray(ss['weight']); za=(xa-center)/scale*weight; zb=(xb-center)/scale*weight; va=np.asarray([scmap[(task,ta['episode'],x['action_index'],'main')]['S_RMS'] for x in ba]); vb=np.asarray([scmap[(task,tb['episode'],x['action_index'],'main')]['S_RMS'] for x in bb]); pa=np.asarray([x['physical_progress_clipped'] for x in ba]); pb=np.asarray([x['physical_progress_clipped'] for x in bb]); mappings={}
    ...
    folda=int(ba[0]['crossfit_fold']); foldb=int(bb[0]['crossfit_fold']); state_cost=mahalanobis_cost(za,zb,.5*(prec[(task,folda)]+prec[(task,foldb)])); raw_state=[tuple(map(int,x)) for x in monotone_match(state_cost,.25)['path']]; mappings['state']=[(i,j) for i,j in raw_state if ba[i]['state_match_accepted'] and bb[j]['state_match_accepted']]
    for method,ia in [('robot_only',robot_idx),('object_only',object_idx)]:
     z1=za[:,ia]; z2=zb[:,ia]; cost=np.linalg.norm(z1[:,None,:]-z2[None,:,:],axis=2); mappings[method]=[tuple(map(int,x)) for x in monotone_match(cost,.25)['path']]
    mappings['unconstrained_nearest_neighbor']=[(i,int(np.argmin(np.linalg.norm(zb-za[i],axis=1)))) for i in range(16)]
    for method,pairs in mappings.items():
     ...
     matches.append({'task':task,'episode_a':ta['episode'],'episode_b':tb['episode'],'method':method,'pair_count':len(pairs),'rejected_pair_count':len(raw_state)-len(pairs) if method=='state' else 0,'pairs':[list(x) for x in pairs],'scalar_spearman':rho(aa,vv),'median_top1_similarity':float(np.median(sims1)) if sims1 else None,'median_top2_similarity':float(np.median(sims2)) if sims2 else None})
